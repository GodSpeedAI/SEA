...
**Language/Runtime**: Python 3.11+ (for Knowledge Graph Service, Artifact Engine, Context Analyzer, Recommendation Algorithm, Domain Services), Node.js 20+ (for CALM CLI Service, Web Application frontend).
**Frameworks**:

- Python: FastAPI (for microservices), Flask (legacy adapters), Neo4j Driver, PyTorch/TensorFlow (for ML models).
- Node.js: React (for Web Application frontend), FINOS CALM library (for CALM CLI).
- Gateway: Kong Gateway (OSS/Enterprise) with declarative configuration and plugin-based policies.
**Databases**:
